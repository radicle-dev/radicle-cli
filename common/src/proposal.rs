...
/// Serializes a proposal to JSON and stores it as git note.
/// Currently, the note needs to be stored not only in the current repository,
/// but also in storage. This will change, as soon as the `remote-helper` allows
...
pub fn store(
    ...
    let spinner = term::spinner("Creating note in working copy.");
    match repo.note(
        &repo.signature()?,
        &repo.signature()?,
        None,
    ...
    let spinner = term::spinner("Creating note in monorepo.");
